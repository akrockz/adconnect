...
def __accountIds():
    if os.path.isfile('config.yaml') is True:
        with open('config.yaml') as f:
            ...
            AccountIds = config[os.environ['PIPELINE_ENVIRONMENT']]['SharedAccounts']
            return AccountIds
    elif os.environ['SHARED_ACCOUNTIDS'] is None:
        raise ValueError('No config or environment variables present, exiting')
    else:
...
def __errorHandler(e):
    if e.response['Error']['Code'] == 'EncryptionFailureException':
        print('Failed to encrypt or decrypt the secret, are you sure you have sufficient KMS privileges?')
        raise e
...
def __sharedSecretPolicy(name, accounts=[]):
    ...
    response = client.put_resource_policy(
        SecretId=name,
        ResourcePolicy=json.dumps(resourcePolicyJson)
    )
    return response['ARN']
...
def put(Name, Value=__generatePassword(), Description='', kmsKeyId='', shared=False):
        try:
            tags = __environmentTags()
            secretName = "{Portfolio}-{App}-{Branch}-{Name}".format(Portfolio=tags[0],
                                                                    App=tags[1],
                                                                    Branch=tags[2],
                                                                    Name=Name)
